...
# ====== CONFIG ======
SCENARIO = "Test"   # Change per run: "Normal", "Fall", "Recovery"
...
while True:
    ...
    if recording_active:
        ...
        if data is not None:
            angle_abs = abs(data["angle"])
            aspect_val = data["aspect_ratio"]

            if aspect_val < 1.2 and 45 <= angle_abs <= 135:
                recovered = True
...
with open("fall_log.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Time", "Angle", "Height", "Aspect", "Fall", "Scenario"])
    writer.writerows(log_data)

print("Log saved as fall_log.csv")
